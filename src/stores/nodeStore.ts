...
export const useNodeStore = defineStore('node', {
  ...
  actions: {
    async loadSettings() {
      ...
      this.randomxFastMode = localStorage.getItem('randomxFastMode') === 'true'; // <--- NEW
    },

    async saveSettings(bin: string, data: string, user: string, pass: string, randomxFastMode: boolean) {
