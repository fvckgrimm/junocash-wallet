...
pub async fn get_block_subsidy(
    ...
    let res = call_rpc("getblocksubsidy", params, &host, port, &user, &pass).await?;
...
#[command]
pub async fn get_mining_info(host: String, port: u16, user: String, pass: String) -> Result<Value, String> {
    // Returns { "localsolps": 12.5, "networksolps": 120000, "difficulty": ... }
    call_rpc("getmininginfo", vec![], &host, port, &user, &pass).await
}
...
pub async fn set_mining(
    ...
    call_rpc("setgenerate", params, &host, port, &user, &pass).await?;
    Ok(if enabled {
        "Mining Started"
    } else {
        "Mining Stopped"
...
pub async fn shield_coinbase(
    ...
    let res = call_rpc("z_shieldcoinbase", params, &host, port, &user, &pass).await?;
